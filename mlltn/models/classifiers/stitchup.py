...
from mllt.models.classifiers.base import BaseClassifier
from mllt.models import builder
from mllt.models.registry import CLASSIFIERS
from mmcv.parallel import DataContainer as DC
import torch
from .utils import union_label, find_common_label


@CLASSIFIERS.register_module
class StitchUpClassifier(BaseClassifier):
    ...
    def __init__(self,
                 ...
                 savefeat=False,
                 **kwargs):
        super(StitchUpClassifier, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(StitchUpClassifier, self).init_weights(pretrained)
    ...
    def extract_feat(self, img, branch=None, need_feat=False, use_bn2=False, **kwargs):
        if self.lock_back:
            with torch.no_grad():
    ...
    def extract_feat_stitchup(self, img1, img2=None, branch=None, need_feat=False, **kwargs):
        if self.lock_back:
            with torch.no_grad():
    ...
    def adjust_labels(self, noisy_labels, prob, branch=None, cls_id=None):
        ...
        alphas = self.random_alphas if branch in ['random'] else self.balanced_alphas
        betas = self.random_betas if branch in ['random'] else self.balanced_betas
    ...
    def forward_train(self,
        ...
        union_labels = union_label(noisy_labels, data2['noisy_labels'])
        common_lbs = find_common_label(noisy_labels, data2['noisy_labels'])
