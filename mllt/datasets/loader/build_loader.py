...
from .sampler import (GroupSampler, DistributedGroupSampler,
                      DistributedSampler, FastRandomIdentitySampler,
                      ClassAwareSampler, NewRandomSampler)
...
def build_dataloader(dataset,
    ...
    if dist:
        ...
        elif 'None' in sampler:
            print('Use random sampler')
            sampler = NewRandomSampler(dataset)
        elif 'SquareRoot' in sampler:
            assert sampler_cfg is not None
            sampler = MixedPrioritizedSampler(dataset, **sampler_cfg)
        elif 'Shift' in sampler:
            assert sampler_cfg is not None
            sampler = ClassPrioritySampler(dataset, **sampler_cfg)
        else:
            raise NameError
