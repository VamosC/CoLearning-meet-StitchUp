...
class NewRandomSampler(RandomSampler):

    def __init__(self, dataset):
        super(NewRandomSampler, self).__init__(dataset)
...
def class_aware_sample_generator(cls_iter, data_iter_list, n, num_samples_cls=1):
    ...
    while i < n:
        ...
        if j == 0:
            cls_id = next(cls_iter)
            temp_tuple = next(zip(*[data_iter_list[cls_id]] * num_samples_cls))
