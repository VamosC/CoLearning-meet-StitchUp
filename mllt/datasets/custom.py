...
class CustomDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.all_imgs = self.load_all_images()
    ...
    def load_all_images(self):
        ...
        for idx in range(len(self.img_infos)):
            img = mmcv.imread(osp.join(self.img_prefix, self.img_infos[idx]['filename']))
            all_imgs.append(img)
        return all_imgs
    ...
    def prepare_train_img(self, idx):
        ...
        noisy_labels = ann['labels']
        if 'clean_labels' in ann:
            clean_labels = ann['clean_labels']
        else:
            clean_labels = np.zeros(1)

        # extra augmentation
        if self.extra_aug is not None:
            img, noisy_labels = self.extra_aug(img, noisy_labels)
        ...
        data = dict(
            ...
            noisy_labels=to_tensor(noisy_labels),
            clean_labels=to_tensor(clean_labels)
        )
        return data
